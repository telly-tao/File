...
CMS是一种 ***以最短回收停顿时间为目标的老年代垃圾收集器***，主要应用于对响应时间敏感的场景（如Web应用）。其核心思想是尽可能让垃圾回收与用户线程并发执行，减少STW（Stop The World）时间。
### 一、CMS工作流程（4个核心阶段）
...
### 三、关键区别
1、时序关系：类初始化是类加载的最后阶段
2、触发条件：类加载时机有JVM决定，而初始化时机有规范严格规定
3、内容差异：
类加载主要完成二进制数据的读取和基础验证
类初始化主要执行静态变量赋值和静态代码块的执行
4、被动引用：某些引用方式（如通过子类访问父类静态字段）会触发类加载但不触发初始化
...
### 二、垃圾回收期选择
G1回收器：-XX:+UseG1GC -XX:MaxGCPauseMillis=200（平衡吞吐与延迟）
CMS回收器：-XX:+UseConcMarkSweeGC（低延迟场景）
...
3、内存管理：可选择固定大小或动态扩展，溢出时抛出OOM错误
4、JDK版本实现变化：
JDK6：采用永久代（PermGen）实现，完全独立于堆内存，存储类信息、静态变量、字符串常量池等
![输入图片说明](/imgs/2026-03-22/6bsX6Q8Ucv9lgMwR.png)
JDK7：开始“去永久代”优化，将字符串常量池和静态变量移至堆内存，但类信息仍保留在永久代
![输入图片说明](/imgs/2026-03-22/zs33vFr0Se5oIge7.png)
JDK8+：彻底移除永久代，改用元空间（Metaspace）实现，使用本地内存存储类元数据，仅字符串常量池保留在堆中
![输入图片说明](/imgs/2026-03-22/tRkakVXZbNDmsHgS.png)
变更原因：
- 永久代大小受限易引发OOM，元空间使用系统内存可动态扩展
- 字符串常量池移至堆内存可受益于堆的GC机制
- HotSpot与其他JVM（如Rockit）实现统一
5、补充
方法区：存储
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTUwODQ1MTIsMTMyNDYyNjM3MCwtMTk5Nj
IxNjM3MiwtMTE5NjE4Njk4NSwtMTM2NzYyMDEzOCwtMTIyNDAz
NzI5MCwtMTE1MDI1MDMyMywtMTAwMDE0NzE5MSwxMDQwOTQ4Mz
MsLTEwNDcyNzk0MjYsLTY3NTg2NDI2NiwtNTk5ODUyOTA1LC05
MDY3MDA4NzYsLTE5MDM3ODk1NzUsLTE0MjE1Nzk1OTUsMTQ3Nj
A0NTAzMiw0MTY5NDM5NTIsLTU5ODg3NTAzMl19
-->