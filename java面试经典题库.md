...
### 一、什么是直接内存？
直接内存（Direct Memory）是Java中一种 ***特殊的内存分配方式***，它不是有Java虚拟机（JVM）直接管理，而是通过Java代码 ***直接向操作系统申请的内存区域***。这部分内存位于JVM堆外，因此也被称为“ ***堆外内存***”（Off-Heap ）

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIzOTMyNTc2MywtMTE5NjE4Njk4NSwtMT
